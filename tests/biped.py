...
bipedDict = {
    'spine': {'numIntervals': 10, 'numJoints': 6, 'axis': 'y', 'upAxis': 'z', 'ctrlInterval': 3, 'reverse': 1},
    'neck': {'numDivisions': 4},
    'limbs': {'numTwistJoints': 4},
    'hand_L': {'base': [pmc.PyNode('hand_L_hrc_gd')],
              'pinky': [pmc.PyNode('hand_L_pinky_%s_gd' % str(num+1).zfill(2)) for num in range(5)],
              'ring': [pmc.PyNode('hand_L_ring_%s_gd' % str(num+1).zfill(2)) for num in range(5)],
              'mid': [pmc.PyNode('hand_L_mid_%s_gd' % str(num+1).zfill(2)) for num in range(5)],
              'index': [pmc.PyNode('hand_L_index_%s_gd' % str(num+1).zfill(2)) for num in range(5)],
              'thumb': [pmc.PyNode('hand_L_thumb_%s_gd' % str(num+1).zfill(2)) for num in range(4)],
    },
    'hand_R': {'base': [pmc.PyNode('hand_R_hrc_gd')],
              'pinky': [pmc.PyNode('hand_R_pinky_%s_gd' % str(num+1).zfill(2)) for num in range(5)],
              'ring': [pmc.PyNode('hand_R_ring_%s_gd' % str(num+1).zfill(2)) for num in range(5)],
              'mid': [pmc.PyNode('hand_R_mid_%s_gd' % str(num+1).zfill(2)) for num in range(5)],
              'index': [pmc.PyNode('hand_R_index_%s_gd' % str(num+1).zfill(2)) for num in range(5)],
              'thumb': [pmc.PyNode('hand_R_thumb_%s_gd' % str(num+1).zfill(2)) for num in range(4)],
    },
...
########################################################################################################################
# Shoulders
shldr_R = componentUtils.addCtrl(spine.endCtrl, pmc.PyNode('body_M_shldr_R_gd'), 'shldr_R', ctrlSize=6)
coreUtils.colorize('red', [shldr_R])

shldr_L = componentUtils.addCtrl(spine.endCtrl, pmc.PyNode('body_M_shldr_L_gd'), 'shldr_L', ctrlSize=6)
...
########################################################################################################################
# Arms
targets = [root.ctrls[-1], spine.baseSubCtrl, spine.endCtrl, spine.startCtrl]
